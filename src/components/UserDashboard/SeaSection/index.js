...
import { updateSeaRequestsData } from "../../../redux/DataRedux";
const SeaSection = () => {
  const InitialData = useSelector((state) => state?.data.data);
  let [Data, setData] = useState([]);
  const SeaData = useSelector((state) => state?.dashboardData.seaRequests);
  console.log(SeaData, "why");
  const onSearch = (value, _e, info) => console.log(info?.source, value);
  const [selectedCat, setSelectedCat] = useState("All");
  const [loading, setloading] = useState(false);
  const dispatch = useDispatch();
  const [all, setAll] = useState(SeaData.all);
  const [active, setActive] = useState(SeaData.active);
  const [unPaid, setUnPaid] = useState(SeaData.unpaid);
  const [paid, setPaid] = useState(SeaData.paid);
  const [timeBounded, setTimeBounded] = useState(SeaData.timebound);
  console.log(Data);
  const fetchData = async () => {
    setloading(true);

    const response = await axios.post(
      `${process.env.REACT_APP_CALCULATOR}userDashboard/GetAllSeaBookingDetails`,
      {
  ...
  return (
    <div style={{ padding: "30px" }}>
      <StyledUserDashBox>
        <h1 style={{ color: "#041c5c" }}>Sea Requests</h1>
      </StyledUserDashBox>
